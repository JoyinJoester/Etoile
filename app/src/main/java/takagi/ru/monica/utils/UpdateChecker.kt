    ...
    fun validateDownloadedApk(context: Context, apkFile: File): Result<Unit> =
        runCatching {
            ...
            if (downloadedPackage.packageName != context.packageName) {
                throw IOException("Downloaded APK package name does not match Etoile")
            }
            ...
            if (!hasMatchingSigner) {
                throw IOException("Downloaded APK signature does not match installed Etoile")
            }
        }
