...
Etoile 是一款专注 Steam 的独立 Android 客户端，源自 [Monica Android](https://github.com/Monica-Pass/Monica-for-Android) 中的 Steam 功能。
它把 Steam 令牌、账号管理、移动确认、游戏库、商城、好友、聊天、通知和 Steam 账号备份集中到一个应用中。

> **当前状态：公开测试版。** Etoile 仍在持续开发中，不是正式版，也不是 Steam 官方客户端。
>
...
1. 从 [Releases](https://github.com/JoyinJoester/Etoile/releases) 下载最新 APK。
...
Etoile 从 Monica Android 的 Steam 功能中独立出来，作为单独产品维护：
...
| Etoile | 独立的 Steam 专用 Android 客户端 | [本仓库](https://github.com/JoyinJoester/Etoile) |

- 独立包名、沙箱、发布周期与仓库：`app.etoile`。
...
- 应用 ID：`app.etoile`，数据库与偏好设置由 Android 应用沙箱隔离。
...
如果 Etoile / Monica 系列对你有帮助，欢迎支持持续开发。
...
Etoile 的设计、兼容性适配与部分功能方向，受到以下优秀开源项目与软件的启发：
